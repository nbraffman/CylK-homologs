...
rm out_homologs_unique_with_Y473.fasta 
...
while read line; do
    if [ $Counter -eq 0 ]; then #copy first line metadata of fasta file to temp.txt
        ...
        for (( i=0; i<${#refseq}; i++ )); do                #iterate through reference sequence (string)
            if [ $i -le $((hash_counter+473)) ]; then       #continue until the index is <= the number of breaks + 105
	...
	echo "Reference sequence residue 473: "${refseq:$((hash_counter+472)):1}

        hash_counter2=0
        index=0
        for (( i=0; i<${#homolog}; i++ )); do              #repeat with homolog sequence (string) for the index length as above
            if [ $i -le $((hash_counter+473)) ]; then      #NOTE this should be hash_counter not hash_counter2
                index=$(($index+1))
                if [ "${homolog:$i:1}" = "-" ]; then        
	...
	#echo "N-terminal breaks in query sequence: "$hash_counter2
        echo "# of residues before Y473 equivalent: "$((index-hash_counter2))
	echo "Query sequence residue Y473: "${homolog:$((hash_counter+472)):1}

        if [ "${homolog:$((hash_counter+472)):1}" = "Y"  ]; then
            filename='temp.fasta'
            while read line; do
                echo $line >> out_homologs_unique_with_Y473.fasta
            done <'temp.fasta'
        fi
    fi
